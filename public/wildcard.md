...
Here’s the very common C-style multi-line comment lexer rule that consumes any characters until it sees the trailing `*/`:

```antlr
COMMENT : '/*' .*? '*/' -> skip ; // .*? matches anything until the first */
```

Without the non-greedy suffix it would match any number of comments (and everything between them) in a single run, while actually you want each comment to be match separately.
...
2. If more than one lexer rule matches the same input sequence, the priority goes to the rule occurring first in the grammar file.
...
3. Nongreedy subrules match the fewest number of characters that still allows the surrounding lexical rule to match.
...
/** 
 * Faster alternate version (Gets an antlr-ng tool warning about
...
constant
    :   'public' 'static' 'final' 'int' Identifier
        {console.log("constant: "+$Identifier.text);}
    ;
